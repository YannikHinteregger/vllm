...
class NixlPushConnectorWorker(NixlBaseConnectorWorker):
    ...
    def __init__(
        ...
        # Engine-main-thread owned. Only failed once the notif count is
        # complete, so every WRITE that did get posted has landed by then.
    ...
    def _do_send_reg_notif(self, req_id: str, reg_data: dict[str, Any]) -> None:
        ...
        # The handshake executor replaces this dict under the lock, so
        # snapshot it rather than iterating it.
    ...
    def _xfer_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        handles: list[int] = []
        # D expects one notif per (producer rank -> consumer rank) edge and
        # the notif rides on the WRITE, so any rank we don't post to must be
        # reported or D waits forever. Ranks we can't resolve stay unreported:
        # a spurious notif would let D finish counting while a sibling
        # producer's WRITE is still in flight.
        ...
        try:
            ...
            if not write_ranks:
                # defaultdict: an evicted engine gives back no ranks
                # instead of raising.
                logger.error(
            ...
            for i, spec in enumerate(read_specs):
                ...
                try:
                    ...
                    # Picking the descriptors can fail on its own, not just
                    # the WRITE.
                    self._log_failure(
    ...
    def _get_new_notifs(self) -> set[str]:
        ...
        while True:
            ...
            # Same body as the completion notif it replaces, so once the
            # marker is off the parsing is shared.
            ...
            if failed_write:
                # Only a consumer can act on this. Here the id collides with
                # one we are sending, and counting it would retire our lease.
                logger.error(
    ...
    def _fail_incomplete_push(self, req_id: str, meta: ReqMeta) -> None:
        ...
        """
        # ``_handle_failed_transfer`` only invalidates blocks for a single
        # group. With none recorded the scheduler treats this as a successful
        # load over a cache that was never written, so leave these alone.
    ...
    def get_finished(self) -> tuple[set[str], set[str]]:
        ...
        # A request retired before its notif count completed would otherwise
        # leave its flag behind forever.
